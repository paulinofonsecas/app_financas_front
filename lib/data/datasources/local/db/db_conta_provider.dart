...
class DbContaProvider implements IContaProvider {
  ...
  @override
  Future<Either<Failure, void>> arquivarConta(int id) async {
    try {
      await initDb();
      var data = _contas.get(id);
      ...
      var conta = Conta.fromMap(data.cast<String, dynamic>());
      conta = conta.copyWith(isArchived: true);
      var map = conta.toMap();
      await _contas.put(conta.id, map);

      return const Right(null);
    } catch (e) {
      return Left(DbException('Erro ao arquivar conta'));
    }
  }

  @override
  Future<Either<Failure, void>> desarquivarConta(int id) async {
    try {
      await initDb();
      var data = _contas.get(id);
      ...
      var conta = Conta.fromMap(data.cast<String, dynamic>());
      conta = conta.copyWith(isArchived: false);
      var map = conta.toMap();
      await _contas.put(conta.id, map);
